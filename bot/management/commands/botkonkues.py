...
def start(m, res=False):
    ...
    # Добавляем две кнопки
    bot.send_message(m.chat.id, starttext)
...
def handle_text(message):
    ...
    if message.text == masterpas:
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        item1 = types.KeyboardButton("Подключить Чат или Группу")
        item2 = types.KeyboardButton("В начало")
        markup.add(item1)
        markup.add(item2)
        bot.send_message(message.chat.id, 'Уговорил, ты в админке', reply_markup=markup)
        return True
