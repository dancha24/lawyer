...
@permission_required('affairs.view_affairs', raise_exception=True)  # Проверка прав
def filterfor(request):
    affairs_all = Affairs.objects.all().filter(date_in__gte=date(2021, 5, 1))
    affairs_per = Affairs.objects.all().filter(date_in__gte=date(2021, 5, 1)).filter(performer=8)
